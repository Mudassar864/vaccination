...
const ComparisonTable = ({
  ...
  const renderSingleEvaluationAnswer = (country, category, questionIndex) => {
    ...
    return (
      <span className={`inline-block px-2 py-1 rounded text-xs font-bold text-center min-w-[40px] ${isYes ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
        {isYes ? (
          <a
            href={linkUrl}
            target="_blank"
  ...
  return (
    <div className="comparison-wrapper text-center py-10 px-5 bg-gray-50 min-h-[60vh]">
      <div className="overflow-x-auto w-full max-w-[1440px] mx-auto relative">
        <table style={{ width: `${tableWidth}px` }} className="min-w-[1440px] border-separate border-spacing-2 my-5 mx-auto font-sans">
          <thead>
            <tr>
              <th style={{ width: "200px" }} className="bg-transparent shadow-none"></th>
                ...
                return (
                  <th key={i} style={{ width: columnWidth }} className="p-2">
                    <select
                      ...
                      style={{
                        backgroundImage: "url(\"data:image/svg+xml;charset=UTF-8,%3csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24' fill='%23f8ac58'%3e%3cpath d='M7 10l5 5 5-5z'/%3e%3c/svg%3e\")",
            ...
            <tr>
              <th style={{ width: "200px" }} className="bg-primary text-white font-bold p-4 rounded-lg sticky left-0 z-[4]">Questions</th>
              {displayCountries.map((country, i) => (
                <th key={i} style={{ width: columnWidth }} className="border-none p-4 bg-white shadow-[0_2px_5px_rgba(0,0,0,0.1)] rounded-lg overflow-hidden break-words">
                  {!country ||
          ...
          <tbody>
              ...
              <React.Fragment key={category}>
                <tr>
                  <td className="text-left align-middle font-bold p-0 bg-primary rounded-lg text-white sticky left-0 z-[3]">
                    ...
                    >
                      <span className={`transition-transform duration-200 ease-in-out mr-1 inline-block ${expandedCategories[category] ? 'rotate-180' : ''}`}>
                        ▼
                      </span>
                      {category}
                    </div>
                  ...
                  {displayCountries.map((country, i) => (
                    <td key={i} className="bg-transparent shadow-none border-none">
                    </td>
                  ))}
                </tr>
                {expandedCategories[category] &&
                  getQuestions(category).map((question, qIndex) => (
                    <tr key={`${category}-${qIndex}`}>
                      <td style={{ width: "400px" }} className="border-none p-4 bg-white shadow-[0_2px_5px_rgba(0,0,0,0.1)] rounded-lg overflow-hidden break-words text-left sticky left-0 z-[3]">
                        {question}
                      </td>
                      {displayCountries.map((country, i) => (
                        <td key={i} style={{ width: columnWidth }} className="border-none p-4 bg-white shadow-[0_2px_5px_rgba(0,0,0,0.1)] rounded-lg overflow-hidden break-words">
            ...
            <tr>
              <td style={{ width: "400px" }} className="text-left align-middle font-bold p-0 bg-primary rounded-lg sticky left-0 z-[3]">
                <div className="block p-4 text-white font-bold text-base">Score</div>
              </td>
              {displayCountries.map((country, i) => (
                <td key={i} style={{ width: columnWidth }} className="border-none p-4 bg-white shadow-[0_2px_5px_rgba(0,0,0,0.1)] rounded-lg overflow-hidden break-words">
